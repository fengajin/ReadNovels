...
namespace ReadNovels.WebApi.Controllers
{
    ...
    /// 小说抓取WebApi
    /// </summary>
    [RoutePrefix("grabNovel")]
    public class CaptureWebApiController : ApiController
    {

        ICaptureService _iCaptureService = null;
        /// <summary>
        /// 构造函数注入点
        /// </summary>
        /// <param name="bookScarkService"></param>
        public CaptureWebApiController(ICaptureService iCaptureService)
        {
            _iCaptureService = iCaptureService;
        }
        /// </summary>
        /// 抓取整本小说(按章节创建TXT文件)
        /// </summary>
        ...
        public int GrabNovelInfo(string getParticularsUrl, string getContentUrl)
        {
            ...
            try
            {
                //引用ReadNovels.Capture，抓取更新小说公共类
                CaptureUpdate_Novel captureUpdate_Novel = new CaptureUpdate_Novel();
                //抓取、更新
                var backCount = captureUpdate_Novel.GrabNovelInfo(getParticularsUrl, getContentUrl);
                if (backCount > 0)
                {
                    return endPrice = 1;
                }
        ...
        public int GrabNovelInfo(string getParticularsUrl, string getContentUrl)
        {
            try
            {
                ...
                if (getParticularsUrl != null)
                {
                    ...
                    //获检查小说是否存在（小说名称、小说作者）
                    var captureById = _iCaptureService.GetCaptures(novelName, noveluthor);
                    ...
                    else
                    {
                        //不存在小说-则添加
                        //保存抓取小说信息
                        int captureBackRow = _iCaptureService.CaptureAdd(capture);
                        endPrice = 1;
                        //调用章节
                        GrabNovelContent(getContentUrl, existData, noveluthor);
                    }
        ...
        public int GrabNovelContent(string getContentUrl, int existData, string noveluthor)
        {
            ...
            try
            {
                if (getContentUrl != null)
                {
                    ...
                    {
                        //获取小说Id
                        var captureIdList = _iCaptureService.GetCaptures(novelName, noveluthor);
                        ...
                        for (int i = 0; i < sMC.Count; i++)
                        {
                            ...
                            //检查此章节是否存在
                            var chapterId = _iCaptureService.GetCaptureSection(captureId, chapterTitle);
                            //不存在
                            if (chapterId.Count == 0)
                            {
                                ...
                                int captureSectionBackRow = _iCaptureService.CaptureSectionAdd(captureSection);//保存抓取小说章节
                                endPrice = 1;
                            }
